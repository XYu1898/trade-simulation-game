...
export function SidebarFooter({ className, children, ...props }: SidebarFooterProps) {
  return (
    <div className={cn("flex items-center justify-between p-4 border-t", className)} {...props}>
      {children}
    </div>
  )
}

const SidebarSeparator = React.forwardRef<React.ElementRef<typeof ScrollArea>, React.ComponentProps<typeof ScrollArea>>(
  ({ className, ...props }, ref) => {
    return (
      <ScrollArea
        ref={ref}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-3 rounded-lg px-3 py-2 text-muted-foreground transition-all hover:text-primary",
  {
    variants: {
      variant: {
